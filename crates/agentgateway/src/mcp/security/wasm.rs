...
impl WasmGuard {
	...
	pub fn new(guard_id: String, config: WasmGuardConfig) -> Result<Self, GuardError> {
		...
		let engine = Engine::new(&engine_config)
			.map_err(|e| GuardError::WasmError(format!("Failed to create wasmtime engine: {}", e)))?;

		// Load and compile the WASM component
		let component = Component::from_file(&engine, expanded_path.as_ref())
	...
	fn create_linker(&self) -> Result<Linker<WasmState>, GuardError> {
		...
		instance
			.func_wrap(
				"log",
				|_store: wasmtime::StoreContextMut<WasmState>, (level, message): (u8, String)| {
					match level {
		...
		instance
			.func_wrap(
				"get-time",
				|_store: wasmtime::StoreContextMut<WasmState>, ()| -> Result<(u64,), wasmtime::Error> {
		...
		instance
			.func_wrap(
				"get-config",
				|store: wasmtime::StoreContextMut<WasmState>,
				 (key,): (String,)|
...
impl NativeGuard for WasmGuard {
	fn evaluate_tools_list(
		...
		self.execute_with_timeout(|| {
			...
			func
				.call(&mut store, &[tools_list, context_record], &mut results)
...
"#;
	...
	fn test_wasm_guard_e2e() {
		...
		let guard =
			WasmGuard::new("test-wasm-guard".to_string(), config).expect("Failed to create WASM guard");
		...
		assert!(
			result.is_ok(),
			"Expected Ok result for safe tool, got: {:?}",
			result
		);
		assert!(
		...
		assert!(
			result.is_ok(),
			"Expected Ok result (not error) for blocked tool"
		);
		match result.unwrap() {
